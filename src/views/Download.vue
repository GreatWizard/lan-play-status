<template>
  <div class="download">
    <h1>Download</h1>
    <h2>LAN Play Client 0.1.0</h2>
    <p>
      <a
        href="https://github.com/spacemeowx2/switch-lan-play/releases/download/v0.1.0/lan-play-win64.exe"
        target="=_blank"
      >
        Windows (64bit)
      </a>
    </p>
    <p>
      <a
        href="https://github.com/spacemeowx2/switch-lan-play/releases/download/v0.1.0/lan-play-win32.exe"
        target="=_blank"
      >
        Windows (32bit)
      </a>
    </p>
    <p>
      <a
        href="https://github.com/spacemeowx2/switch-lan-play/releases/download/v0.1.0/lan-play-macos"
        target="=_blank"
      >
        MacOS
      </a>
    </p>
    <p>
      <a
        href="https://github.com/spacemeowx2/switch-lan-play/releases/download/v0.1.0/lan-play-linux"
        target="=_blank"
      >
        Linux
      </a>
    </p>
    <h2>Switch Sysmodule and Homebrew (ldn_mitm)</h2>
    <p>
      <a
        href="https://github.com/spacemeowx2/ldn_mitm/releases/download/v1.2.3/ldn_mitm_v1.2.3.kip"
        target="=_blank"
      >
        Ldn_mtm v1.2.3
      </a>
    </p>
    <p>
      <a
        href="https://github.com/spacemeowx2/ldn_mitm/releases/download/v1.2.3/ldnmitm_config.nro"
        target="=_blank"
      >
        Ldn_mtm config homebrew v1.2.3
      </a>
    </p>
    <em> (or you can use SX-OS v2.5+) </em>
  </div>
</template>
