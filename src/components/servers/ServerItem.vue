<template>
  <span class="contents" v-if="status >= 0">
    <tr :class="trClasses">
      ...
      <td :data-tooltip="infos">
        <span v-if="data.active !== undefined && data.idle !== undefined">
          <span class="inline-block--on-mobile">
            {{ data.active }}
            <img alt="Active users" class="icon" src="@/assets/icons/active.png" />
          </span>
          <span class="hide--on-mobile"> / </span>
          <span class="inline-block--on-mobile">
            {{ data.idle }}
            <img alt="Idle users" class="icon" src="@/assets/icons/idle.png" />
          </span>
        </span>
        <span v-else>
          <span>
            {{ data.online }}
            <img alt="Online users" class="icon" src="@/assets/icons/online.png" />
          </span>
        </span>
      </td>
      ...
      <td>
        <span v-if="ping >= 0"> {{ ping }}<span class="hide--on-mobile"> ms</span> </span>
        <span v-else>n/a</span>
      </td>
      <td>
        <span v-if="uptime"> {{ uptime }}<span class="hide--on-mobile">%</span> </span>
        <span v-else>n/a</span>
      </td>
    </tr>
  ...
  computed: {
    ...
    uptime() {
      let monitor =
        this.monitors &&
        this.monitors.find(
          (monitor) => monitor.friendly_name === `${this.server.ip}:${this.server.port}`,
        )
