...
const AdvertiseDataMap = (_gameId) => {
  switch (_gameId) {
    ...
    case '01008f6008c5e000':
      return (data) => {
        const code = `${data.charAt(185)}${data.charAt(187)}${data.charAt(189)}${data.charAt(191)}`
        return {
          code,
        }
      }
...
const sanitizeData = (_room) => {
  if (_room.contentId === 'ffffffffffffffff') {
    if (_room.advertiseData === 'ffff0000') {
    ...
    } else if (_room.advertiseData.startsWith('7b') && _room.advertiseData.endsWith('7d')) {
      // Rocket League
      _room.contentId = '01005ee0036ec000'
      ...
      do {
        ...
        if (!Number.isNaN(readingSize) && readingSize > 0) {
          cursor += 4
          const characterName = hexToUtf16(`${players.substr(cursor, readingSize)}0000`)
          cursor += readingSize
          newNodes.push({
