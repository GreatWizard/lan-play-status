<template>
  ...
  <p>
    ...
    >
      WinPcap (note: this says winpcap, not npcap)</a
    >.
  </p>
  <p>
  ...
  <p>
    First, you need to install
    <a href="https://brew.sh/" target="_blank" rel="noreferrer noopener">Homebrew</a>.
  </p>
  <p>Then in a terminal: <code>brew install libpcap libuv switch-lan-play</code>.</p>
...
Server IP: 149.91.81.201"
  ...
  <p>
    Finally in a terminal: <code>sudo ./lan-play-linux</code> (ensuring you are in the proper
    directory (likely ~/Downloads).
  </p>
  <TerminalText
...
Server IP: 149.91.81.201"
  />
  <p>
    If you omit the <code>--relay-server-addr</code> option, the application will prompt you to
    enter the relay server address. You can choose the server you want from our
    <a href="http://lan-play.com/">list of servers</a> or any other server you may know of.
  </p>
  <p>
    You need to run the client administratively and possibly allow it through your firewall. If the
    client exits right away, ensure you are running as an administrator or root and pasting in the
    server correctly.
  </p>
  <h2>{{ $t('install.games.title', { prefix: '3 - ' }) }}</h2>
  <p>
    Launch the desired game you want to play (must have the LAN play feature).
    <strong>LAN and Local wireless are different things.</strong>
  </p>
  <p>
